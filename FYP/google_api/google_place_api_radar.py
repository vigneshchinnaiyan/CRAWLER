...
def write_to_json_file(google_radar_data):
    f = open('google_place_api_radar_data.json', 'w')
    json.dump(google_radar_data, f, indent=4) 


def google_radar_place_ids():
	"""
	The Google Places API Radar Search Service allows you to
	search up to 200 places at once, and identify specfic areas of
	interest within a geographic area.

	To find the different types of venues, you can go here to check
	out more supported types:
	https://developers.google.com/places/web-service/supported_types

	A Radar requires at least one of the three:
	1) keyword
	2) name
	3) type
	"""
	...
	industry_type = "bank"
	google_url = "https://maps.googleapis.com/maps/api/place/radarsearch/json?location=" + latitude + "," + longitude + "&radius=" + str(radius) + "&key=" + GOOGLE_API_KEY + "&type=" + industry_type
...
def main():
	...
	place_ids = google_radar_place_ids()
	google_radar_data = []
	number_of_venues = 20
	...
	for i in xrange(number_of_venues):
		...
		google_radar_data.append(contact)

	write_to_json_file(google_radar_data)
